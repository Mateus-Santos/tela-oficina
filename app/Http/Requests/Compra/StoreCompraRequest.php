...
class StoreCompraRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $anexos = array_filter($anexos, function ($anexo, $index) {
            $arquivo = $this->file("anexos.{$index}.arquivo");

            return $arquivo instanceof \Illuminate\Http\UploadedFile
                || !empty($anexo['tipo'] ?? '')
    ...
    public function messages(): array
    {
        return [
            ...
            'anexos.*.observacoes.string' => 'As observações do anexo são inválidas.',
            'anexos.*.observacoes.max' => 'As observações do anexo não podem ter mais de 1000 caracteres.',
        ];
    }
}
