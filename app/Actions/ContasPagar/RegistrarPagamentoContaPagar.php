...
{
    public function execute(ContaPagar $conta, array $dados): PagamentoContaPagar
    {
        return DB::transaction(function () use ($conta, $dados) {
            ...
            if ($conta->status === 'cancelada') {
                throw new InvalidArgumentException('Não é possível registrar pagamento em uma conta cancelada.');
            }
            ...
            $saldo = round((float) $conta->valor - $valorPago, 2);

            if ($saldo <= 0) {
                throw new InvalidArgumentException('Esta conta já está totalmente paga.');
            }
            ...
            $novoValorPago = round($valorPago + $valor, 2);
