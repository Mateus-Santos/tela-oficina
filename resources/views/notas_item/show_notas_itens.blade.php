...
<div class="container cadastro">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            @if($nota->status === 'Aberto' && auth()->user() && auth()->user()->permitions != 2)
                <form action="{{ route('notas.finalizar', $nota->id) }}" method="POST" onsubmit="return confirm('Deseja finalizar esta nota? Os produtos serão baixados do estoque e a nota não poderá mais ser editada.');">
                    @csrf
            ...
            @if($nota->status === 'Finalizado' && auth()->user() && auth()->user()->permitions != 2)
                <form action="{{ route('notas.cancelar', $nota->id) }}" method="POST" onsubmit="return confirm('Deseja cancelar esta nota? Os produtos serão devolvidos ao estoque e esta operação não poderá ser desfeita.');">
                    @csrf
    ...
    <table class="table">
        ...
        <tbody>
            <tr>
                ...
                <td>
                    <a href="{{ route('notas.pdf', $nota->id) }}" target="_blank" class="btn btn-danger" title="Gerar PDF">
    ...
    @if($itens->isEmpty())
        <div class="alert alert-info d-flex justify-content-between align-items-center">
            ...
            @if($nota->status === 'Aberto' && auth()->user() && auth()->user()->permitions != 2)
                <a class="btn btn-success" href="{{ route('notasitem.edit', $nota->id) }}">
                ...
                <tbody>
                    @foreach($itens as $item)
                        @php
                            ...
                            $subtotalItem = max(0, ($quantidade * $valorUnitario) - $desconto);
                        @endphp
                        ...
                        <tr>
                            ...
                            @if($nota->status === 'Aberto' && auth()->user() && auth()->user()->permitions != 2)
                                <td>
                                    <form action="{{ route('notasitem.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Deseja realmente remover este item da nota?');">
                                        @csrf
                                        @method('DELETE')
